...
export function fabricSVG(draft,back=false,scale=12){
 ...
 for(let r=0;r<draft.picks;r++)for(let c=0;c<draft.cards;c++){
  ...
  let end=r+1;
  if(sparse[c])while(end<draft.picks&&data[end][c].hole===p.hole)end++;
...
return {...draft,...(draft.startHole===undefined?{}:{startHole:Math.min(draft.startHole,holes-1)}),holes,cards,picks,threads:Array.from({length:cards},(_,c)=>Array.from({length:holes},(_,h)=>(h<draft.holes?draft.threads[Math.min(c,draft.cards-1)][h]:COLORS[2].hex))),slants:Array.from({length:cards},(_,c)=>draft.slants[Math.min(c,draft.cards-1)]),turns:Array.from({length:picks},(_,r)=>Array.from({length:cards},(_,c)=>draft.turns[r%draft.picks][Math.min(c,draft.cards-1)]))};
}
...
const {holes,cards,picks,colors,threads,slants,turns,startHole}=input;
...
return {version:1,...(startHole===undefined?{}:{startHole}),name:input.name||'Untitled pattern',holes,cards,picks,colors:colors.map(c=>({name:c.name,hex:c.hex.toLowerCase()})),threads:threads.map(r=>r.map(h=>h===null?null:h.toLowerCase())),slants:[...slants],turns:turns.map(r=>[...r])};
}
...
let body=`<rect width="100%" height="100%" fill="white"/>${txt(28,34,draft.name,24)}${txt(28,60,`${draft.holes} holes · ${draft.cards} tablets · ${draft.picks} picks · ${threadCount(draft)} warp threads`)}${txt(28,80,'One hole per turn. F = away; B = toward you. Pick 1 is at the top.',11)}`;
...
body+=txt(28,height-33,`Start with ${String.fromCharCode(65+mod((draft.startHole??draft.holes-1)+1,draft.holes))} upper-far, ${String.fromCharCode(65+(draft.startHole??draft.holes-1))} upper-near. Read the labelled face from the right.`,11)+txt(28,height-15,'S/Z describes threading. Preview is schematic; sample to check yarn, floats, and tension.',11);
