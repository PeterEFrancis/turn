import { PATTERNS, getPattern, createPreset } from './patterns.js?v=5';
import {COLORS,createDraft,weave,fabricSVG,escapeXML,mod,validateDraft,resizeDraft,chartSVG} from './model.js?v=4';
...
let threading='<span></span>'+Array.from({length:draft.cards},(_,c)=>`<span class="chart-label">${c+1}</span>`).join('');for(let h=0;h<draft.holes;h++){threading+=`<span class="chart-label">${String.fromCharCode(65+h)}</span>`;for(let c=0;c<draft.cards;c++)threading+=`<button class="hole-cell" data-card="${c}" data-hole="${h}" style="--thread:${draft.threads[c][h]}" aria-label="Tablet ${c+1}, hole ${String.fromCharCode(65+h)}, ${draft.threads[c][h]}" title="Tablet ${c+1} · Hole ${String.fromCharCode(65+h)}"></button>`;}
...
const color=draft.colors.find(c=>c.hex===activeColor);$('#active-color-name').textContent=color?.name||activeColor;$('.mini-swatch').style.background=activeColor;
...
$('#palette').addEventListener('click',e=>{const b=e.target.closest('[data-color]');if(b)setColor(b.dataset.color);});
